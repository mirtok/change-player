...
<div class="footer-container">
    ...
    <div class="footer">
        ...
        <div class="middle">
            <span :style="{'color':currentVideo?'':'#454548'}" :title="$t('common.stop')" class="fa fa-stop" @click="stop"></span>
            <span :style="{'color':(videoList.length<=1)?'#454548':''}" @click="prev" :title="$t('common.previous')" class="fa fa-step-backward"></span>
            <span @click.stop="switchPlaying(true)" v-if="!isPlaying" :title="$t('common.play')" class="fa fa-play-circle-o" style="font-size: 50px;"></span>
            <span v-if="isPlaying" :title="$t('common.suspend')" class="fa fa-pause-circle-o" style="font-size: 50px;" @click.stop="switchPlaying(false)"></span>
            <span :style="{'color':(videoList.length<=1)?'#454548':''}" @click="next" :title="$t('common.next')" class="fa fa-step-forward"></span>
            <span :title="$t('common.mute')" v-if="isVolumeOn" style="width: 40px;height: 20px;" @click.stop="setVolume(false)" class="fa fa-volume-up"></span>
            <span :title="$t('common.cancelMute')" v-if="!isVolumeOn" style="width: 40px;height: 20px;" @click.stop="setVolume(true)" class="fa fa-volume-off"></span>
            <my-progress ref="progress" width="62px" />
        </div>
        <div class="right">
            <span :title="$t('common.noTrace')" class="fa fa-snowflake-o" :style="{'color':isTrace?'#1bb017':''}" @click="noTrace"></span>
            <span :style="{'color':currentVideo?'':'#454548'}" @click="fullScreen" :title="isFullScreen?$t('common.exitScreen'):$t('common.fullScreen')" class="fa fa-expand my-expand"></span>
        </div>
    </div>
</div>
</template>
...
export default {
    ...
    computed: {
        ...
        playModeList() {
            return [{
                    title: this.$t('common.singlePlay'),
                    playMode: 1
                },
                {
                    title: this.$t('common.singleCycle'),
                    playMode: 2
                },
                {
                    title: this.$t('common.loopList'),
                    playMode: 3
                },
                {
                    title: this.$t('common.sequentialPlay'),
                    playMode: 4
                },
                {
                    title: this.$t('common.randomPlay'),
                    playMode: 5
                }
            ]
        }
